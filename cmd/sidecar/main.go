...
func main() {
	...
	log := logger.Init().With("service", "LogsSidecar")
	...
	// Event bus
	natsConn := Must(NewNatsConnection(log, cfg.NatsURI))
	defer func() {
...
func streamLogs(ctx context.Context, l *zap.SugaredLogger, id, ns string, logs chan events.LogChunk) (err error) {
	...
	for _, pod := range pods.Items {
		...
		case corev1.PodRunning:
			...
			return streamLogsFromPod(l, podsClient, ns, pod.Name, logs)
...
func streamLogsFromPod(log *zap.SugaredLogger, podsClient tcorev1.PodInterface, namespace string, podName string, logs chan events.LogChunk) (err error) {
	defer close(logs)
